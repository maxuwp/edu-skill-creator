...
3. Does the source state any number I might be quoting — a count, a threshold, an effect size, a
   duration, a sample size? Report it exactly. If a number in circulation cannot be traced to the
   source, report it as untraceable rather than approximating it.
...
book's official sample pages, the project's own repository or documentation.
...
1. **Understanding by Design (Wiggins & McTighe).** We use it to ground planning from outcomes and
   content prioritization. Our scope limit: "Curriculum planning; not a delivery or slide-design
   method."

2. **Mager / ABCD objective format.** We use it to ground writing measurable objectives. Our scope
   limit: "Objective wording; says nothing about sequencing."

3. **Gagné's nine events of instruction.** We use it to ground session event sequencing. Our scope
   limit: "Event arc for a session; not curriculum-level planning."

4. **Merrill's first principles of instruction.** We use it to ground problem-centred instruction.
   Our scope limit: "Task-centered instruction; complements, not replaces, Gagné."
