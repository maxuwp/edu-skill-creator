...
in `<edu-skill-creator-skill-dir>/reference/lesson_index.md` and its detail files (already-paid-for RED).
...
`<repo>/tests/run_deterministic.py` (seconds, no model calls; the case floor lives in
`MIN_SUITE_CHECKS` — never restate the count here, it drifts) and `tests/evals/E1`–`E2`
