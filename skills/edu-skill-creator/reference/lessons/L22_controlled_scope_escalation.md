...
of record, its layer, the breadth it must cover completely, its budget, and who may enact an
expansion. A finding whose fix would land below the declared layer is **scope pressure**, reported
separately and never fixed in the same pass. Expansion is then classified by impact and routed to
the matching authority: a local delta the author enacts; a controlled descent the author enacts and
**records**; a major re-scope or a relocation the faculty member authorizes. A round may spend **one**
controlled descent; a second converts the round into a major re-scope proposal. The runnable
procedure, with the declaration block, the scope-pressure record and the brief templates, is
`<edu-skill-creator-skill-dir>/reference/review_scope_protocol.md`.
...
never fall for a reason that has nothing to do with quality. The rule is order, not effort —
**complete the declared breadth at the declared layer first, then report once.** A reviewer who
returns one new issue per round, each deeper than the last, is producing descent and it will read
as diligence.
...
block, and nothing detects that a fix touched below the declared layer. Treat an unwired rule as
