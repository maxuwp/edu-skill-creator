...
3. **Drafter ≠ reviewer (L3).** Applies to the new plugin's stages AND to Edu Skill Creator itself:
   drafted skills are reviewed by a fresh subagent session against
   `<edu-skill-creator-skill-dir>/reference/skill_quality_rubric.md`.
4. **Human gates (L5).** Present each Edu Skill Creator gate to the author and wait. Never approve on
...
7. **Edu Skill Creator's own artifacts obey the stale-state rule (L4).** Before dispatching a stage,
   ...
   and its stages re-run before the pipeline continues. The discipline Edu Skill Creator designs
   into other plugins applies to its own build.
   (L4's gate-splitting and L7's single-source rules are enforced structurally by
   edu-skill-creator-architecture and edu-skill-creator-scaffold rather than repeated here.)
8. **Accessible gates.** When Edu Skill Creator presents its own gates through generated HTML pages
...
|---|---|---|---|
| 1 Intent | `edu-skill-creator-intent` | `intent.md` + contested-choices inventory | author approves intent |
| 2 Grounding | `edu-skill-creator-grounding` | `grounding_frameworks.md` (new plugin's map) | author approves map; unanchored stages resolved |
| 3 Architecture | `edu-skill-creator-architecture` | `architecture.md` + `BUILD_PLAN.md` + manifest/dependency schema | author approves design |
| 4 Scaffold | `edu-skill-creator-scaffold` | dual-harness repo skeleton + lint + link script | lint exits 0 |
...
`edu-skill-creator-refresh` sits outside the numbered sequence deliberately: it is periodic
...
## What Edu Skill Creator is not

Edu Skill Creator builds *educational* plugins. For a generic (non-educational) plugin, point the
author at Anthropic's skill-creator and the official plugin-dev plugin instead — Edu Skill Creator's
