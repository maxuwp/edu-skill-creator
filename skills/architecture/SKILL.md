...
6. **User-decision tracing (L15), process fidelity (L17), governed exits (L18).** Name how an
   explicit user decision is carried to every stage able to alter it, and which stage checks it —
   the lightest mechanism that gives traceability for this product type, examples in
   `<edu-skill-creator-skill-dir>/reference/implementation_patterns.md`. Where an outcome is
   process-shaped, name the sourced representation of that process and the check that the teaching
   sequence walks it. And every stage that can fail names its governed blocked-or-escalate outcome,
   so the compliant route is never costlier than the workaround.
7. **Contested choices at intake (L2).** Place every inventoried choice as an explicit
...
8. **Cost ladder (L6).** Mark token-expensive stages (simulations, sweeps) with their
   full/lite/skip consent design and the manifest field recording the mode.
9. **Data-flow & security model (mandatory when applicable).** If the plugin handles
   ...
   an afterthought.
10. **Component inventory.** Beyond skills: agents, gate app (adapt POSED's guided app —
...
11. **Lifecycle stages (L9).** Include reflect and refresh stages in the design by
   default; the author may decline them at the gate (their call — L2 applies to us too).
12. **Computed-validation plan (L11).** For every artifact, list which requirements are
   ...
   `reference/validator_template.py`, so this plan only needs to name the artifacts
