# AGENTS.md — PAGE (Codex + Claude Code)

This repo is the **single source of truth** for PAGE on both harnesses. It ships the
Claude Code plugin (`.claude-plugin/`) and the Codex plugin (`.codex-plugin/`) from one
shared `skills/` set.

**The personal skill dirs are symlinks into this repo.** `<skills-dir>/page-<x>` →
`skills/<x>` (the `page-` prefix is dropped here; the umbrella `page` keeps its name).
...
5. **Keep SKILL bodies tool-agnostic.** No literal harness paths — use
   `<page-skill-dir>/…` / `<skills-dir>/…` per
   `skills/page/reference/harness_adaptation.md`. Say "a fresh subagent session," not a
   harness-specific API name.
6. Bump BOTH plugin.json files in lockstep on release; add a real
   `## page_skill.X.Y — <date>` heading to CHANGELOG.md.
...
Full detail: **MAINTAINING.md** and `skills/page/reference/dual_harness_playbook.md`.
